...
**Inspected at Phase 4A:** `ios/Runner.xcodeproj/project.pbxproj` → `IPHONEOS_DEPLOYMENT_TARGET = 13.0` (all three Runner build configurations: Debug, Release, Profile). **Current:** the Runner and `EastWidgetExtension` targets both support **iOS 15.0**. The widget uses availability-gated fallbacks for its iOS 16/17-only presentation modifiers; this does not change the operation-based CloudKit decision below.
...
This section records the original Phase 4A shape of the Dart-to-Swift boundary. The implemented architecture ultimately kept the native side as one-shot CloudKit transport primitives and placed cycle ownership in `SyncOrchestrator`, the integration coordinators and `CloudKitSyncRuntimeCoordinator`. The provisional, never-implemented `SyncEngine` port and its test-only fake were removed once this concrete architecture superseded them.

| Capability | Direction | Implemented shape |
|---|---|---|
...
| Modify records | Dart → Swift → Dart | One-shot request/response carrying validated wire envelopes. Dart owns outbox state, conflict decisions and result application. |
| Fetch remote changes | Dart → Swift → Dart | One-shot paginated fetch returning validated projections and the next server token. Dart owns local application and durable checkpointing. |
| Start/request sync | Dart internal | `CloudKitSyncRuntimeCoordinator` owns launch, foreground, account-change and retry triggers; it composes the orchestrator and integration coordinators rather than asking Swift to run a cycle. |
| Enqueue local change | Dart internal | Durable `LocalSyncIntentStore` and `SyncPersistenceStore` writes; nothing is sent to Swift until an orchestrated transport pass. |
| Account-change events | Swift → Dart | Content-free `CloudKitAccountChangeEvent` marker; Dart requests a fresh account snapshot and enforces the association boundary. |
| Retryable vs. permanent errors | Swift → Dart | Every failure response carries a symbolic error code Dart can classify via `lib/sync/sync_error_classification.dart` — Dart owns the retry/backoff policy (§3); Swift only reports what happened, using CloudKit's own retry hint where available. |
...
All of the following live under `lib/sync/`, are pure (no `dart:io`, no platform channel, no network), and are fully unit-testable without a device, simulator, or CloudKit access. None duplicates `KeptRecord` as a second competing domain model — every type here is either a pure projection *of* `KeptRecord`/`SyncTombstone` or an independent sync-only concern.
...
6. **`lib/sync/sync_error_classification.dart`** — `SyncErrorCategory` (`retryable`/`permanent`/`accountIssue`) and `classifySyncErrorCode(String symbolicCode)`, a pure lookup with no dependency on any real CloudKit error type.
7. **`lib/sync/conflict_resolution.dart`** — `ConflictCandidate` (projection + its role as local/remote), `ConflictOutcome` (winner + `ConflictReason`), and `resolveKeptWisdomConflict({required local, required remote, required authoritativeEpoch})` — the pure, deterministic implementation of §4.1's precedence order, including the immutable-field-mismatch fail-closed check (§4.2).

Runtime status and account-change models now live with the concrete bridge/runtime layers (`lib/sync_platform/` and `lib/sync_runtime/`) rather than in an unused generic engine abstraction.
...
- **`lib/sync_platform/cloud_kit_platform_bridge.dart` (`CloudKitPlatformBridge`) remains a narrow transport boundary.** It never owns cycle ordering, conflict application, durable retries, or local persistence; the implemented orchestrator, integration and runtime layers compose it explicitly.
...
| (6) Support partial failure without treating successful records as failed | Every requested record's own outcome is collected independently via the typed iOS 15 `CKModifyRecordsOperation.perRecordSaveBlock` / `modifyRecordsResultBlock` callbacks. A record CloudKit never got to attempt is never reported as failed; see 12.4. |
